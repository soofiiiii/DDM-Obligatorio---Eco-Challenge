...
export default function HomeScreen() {
  ...
  const testNotification = async () => {
    ...
    if (!hasPermission) {
      Alert.alert(
        'Permiso Denegado',
        'No se pudo programar la notificación de prueba porque no hay permisos.'
      );
      return;
    }
    ...
    try {
      ...
      Alert.alert(
        ...
        `Se programó una notificación para ${scheduledTime.toLocaleTimeString()}. Minimiza la app para verla en 5 segundos.`
      );
    } catch (error) {
    ...
    >
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <View style={styles.actionsContainer}>
          ...
          <TouchableOpacity
            style={[styles.actionButton, { backgroundColor: COLORS.accentBlue, marginTop: 20 }]}
            onPress={testNotification}
          >
            <Icon name="bell" size={30} color={COLORS.white} style={styles.actionIcon} />
